...
Division of labour (same as deepdive handoff):
  * THIS script (cheap/mechanical, OK on VPS/private-runtime): inventory which KB patterns
  ...
  * Template GENERATION is a LOCAL strong-model (Opus) job — converting prose
    ...
    or false-positive). A private-runtime agent MUST NOT generate templates.
...
def main():
    ...
    if gaps:
        print("\n  → 本地 Opus: automation/pull_nuclei_gaps.sh 後用 bb_kb_to_nuclei.py 生成。"
              "\n  → 切勿讓 private-runtime(minimax) agent 生成模板。")
