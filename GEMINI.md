...
Claude Code ships 5 specialized agents in `.claude/agents/` (attack-chain-deep-dive, bbflow-runner, pre-recon, report-writer, vault-sync). Gemini CLI does not run these natively, but their prompts are plain Markdown you can load on demand. The generated `bb-agent-prompts` skill routes common requests (e.g. "run hunters", "pre-recon") to the right agent prompt under `.claude/agents/*.md`; CVSS / severity scoring routes to the `bb-cvss-score` skill (`.claude/skills/bb-cvss-score/SKILL.md`), not an agent.
