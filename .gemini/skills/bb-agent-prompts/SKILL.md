...
description: Use when asked to use project Claude agents, agent-team-orchestrator, attack-chain-deep-dive, auto-poc-gen, bbflow-runner, chain-tracker, cve-monitor, cve-pattern-cross-gen, debug-leak-scanner, disclosed-report-researcher, endpoint-interest-scorer, js-sourcemap-miner, lessons-miner, nuclei-template-gen, pre-recon, recon-diff, regression-tester, report-writer, sandbox-replay, skill-synthesizer, submit-form, triage-classifier, vault-sync, web-hunter, or to mirror bug bounty agent behavior in Gemini CLI.
---
