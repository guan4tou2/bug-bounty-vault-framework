...
But running Ring 4 is necessary, not sufficient. The `① → ③` edge ("informs hunting") is the *other* half: captured knowledge only helps if it is actually **retrieved** and produces findings. Capture is easy to measure (did Ring 4 run?); retrieval is usually assumed and never measured, so the wiki grows unbounded with no evidence of which pages help. The **effect loop** closes that gap with three forward measurements: `automation/kb_roi.sh` + a Finding `helped_by:` field (which KB artifacts actually produced findings — credited vs prune-candidate), `automation/surface_kb.sh` (fingerprint → point-of-need KB so the `① → ③` edge fires at the right moment), and `automation/check_shelfware.sh` (templates that never reach Ring 3). Mechanical detection, LLM judgment — uncredited / zero-instance is a prune *candidate*, not an auto-delete.
...
## 3. Three failure modes this design prevents

All are *silent* — nothing errors, you just quietly get a worse result. The gates exist because these are the failures real sessions repeatedly hit.
...
| **Capture without retrieval** | Ring 4 runs, the wiki/templates grow, but nothing is ever retrieved or produces a finding — shelf-ware accumulates with no evidence to prune | effect loop: `kb_roi.sh` (Finding `helped_by:` provenance) + `surface_kb.sh` (point-of-need) + `check_shelfware.sh` (unused-template detection) |
...
- [ ] Effect loop: each confirmed finding recorded its `helped_by:` (which KB led to it), so retrieval — not just capture — is measurable.
